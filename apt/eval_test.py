...
if __name__ == '__main__':
    ...
    if os.path.exists(progress_file_path):
        print(f"*** Tìm thấy file progress, đang tải lại: {progress_file_path} ***")
        try:
            ...
            print(f"*** Tải thành công! Tiếp tục từ batch {start_batch} ***")
        except Exception as e:
            print(f"Lỗi khi tải file progress, bắt đầu lại từ đầu. Lỗi: {e}")
            start_batch = 1
            meters = Dict()
            meters.acc = AverageMeter('Clean Acc@1', ':6.2f')
            meters.rob = AverageMeter('Robust Acc@1', ':6.2f')
    ...
    for i, data in enumerate(loader, start=1):
        ...
        if i == 1 or i % 10 == 0 or i == len(loader):
            ...
            # --- THÊM LOGIC LƯU CHECKPOINT PROGRESS ---
            print(f"*** Đang lưu progress tại batch {i} vào {progress_file_path} ***")
            progress_data = {
                'last_batch': i,
            ...
            try:
                # Ghi đè file progress
                with open(progress_file_path, 'w') as f:
                    json.dump(progress_data, f)
            except Exception as e:
                print(f"Cảnh báo: Không thể lưu file progress. {e}")
    ...
    # --- THÊM LOGIC DỌN DẸP FILE PROGRESS ---
    if os.path.exists(progress_file_path):
        try:
            os.remove(progress_file_path)
            print(f"*** Đánh giá hoàn tất. Đã xóa file progress: {progress_file_path} ***")
        except OSError as e:
            print(f"Cảnh báo: Không thể xóa file progress. {e}")