\documentclass[a4paper, oneside]{project}
\usepackage[utf8]{vietnam}
\usepackage{placeins}
\usepackage{amsfonts}
\usepackage{textcomp}
\usepackage{dblfloatfix}
\usepackage{subfig}
\usepackage{lineno}
\usepackage{amssymb}
\usepackage{tabularx,booktabs}
\usepackage{longtable, lipsum}
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage{graphicx}
\usepackage{lscape}
\usepackage{caption}
\usepackage{hyperref}
\usepackage[sorting=nyt]{biblatex}
\usepackage{mathtools}
\usepackage{lipsum}
\usepackage{fancyhdr}
\usepackage{titlesec}
\usepackage{makecell}
\usepackage{blindtext}
\usepackage{subfiles} % Chèn các file nhỏ, giúp chia các chapter ra nhiều file hơn
\usepackage{algorithm}
\usepackage{algpseudocode}
\usepackage{multirow}


% Config reference
\addbibresource{ref.bib}

% Setup hype link in thesis
\hypersetup{urlcolor=blue,linkcolor=black,citecolor=black,colorlinks=true} 
\DeclarePairedDelimiter\bra{\langle}{\rvert}
\DeclarePairedDelimiter\ket{\lvert}{\rangle}
\DeclarePairedDelimiterX\braket[2]{\langle}{\rangle}{#1 \delimsize\vert #2}


% Config information thesis
\upperuniname{ĐẠI HỌC QUỐC GIA THÀNH PHỐ HỒ CHÍ MINH}
\uniname{TRƯỜNG ĐẠI HỌC CÔNG NGHỆ THÔNG TIN}
\deptname{KHOA KHOA HỌC MÁY TÍNH}
\stumajor{KỸ SƯ KHOA HỌC MÁY TÍNH}
\title{TĂNG CƯỜNG TÍNH BỀN VỮNG TRƯỚC TẤN CÔNG ĐỐI KHÁNG CHO CÁC MÔ HÌNH
THỊ GIÁC-NGÔN NGỮ ĐƯỢC TIỀN HUẤN LUYỆN THÔNG QUA TINH CHỈNH GỢI Ý ĐỐI KHÁNG}
\titleen{Enhancing Adversarial Robustness of Pretrained Vision--Language Models via Adversarial Prompt Tuning}
\supervisor{GIẢNG VIÊN HƯỚNG DẪN}
\supervisorname{TIẾN SĨ LƯƠNG NGỌC HOÀNG}
\stunamewithid{LÊ MINH NHỰT - MSSV 22521060\\TĂNG NHẤT - MSSV 22521027}
\reporttime{NĂM 2026}

% Begin thesis
\begin{document}

\coverpage

% % Begin above main thesis
\frontmatter
\subfile{chapters/front/thanks}
\clearpage
\tableofcontents
\clearpage
\listoffigures
\clearpage
\listoftables
\clearpage
\subfile{chapters/front/glossaries}


% Begin main thesis, start page numbering
\counterwithin{equation}{section}
\counterwithin{table}{section}
\counterwithin{figure}{section}
\setcounter{secnumdepth}{3}
\mainmatter
\fancyhf{}
\fancyfoot[C]{\thepage}
\subfile{chapters/main/summary}
\subfile{chapters/main/intro}



% Config page header
\if @twoside
  \fancyhead[EL,OR]{\bfseries\nouppercase\rightmark}
\else
  \fancyhead[R]{\bfseries\nouppercase\rightmark}
\fi

% Main chapter in thesis
\subfile{chapters/main/chapter-1}
\subfile{chapters/main/chapter-2}
\subfile{chapters/main/chapter-3}


% Conclusion
\subfile{chapters/main/conclusion}
% Print references
\printbibliography[heading=bibintoc, title = {Tài liệu tham khảo}]
\end{document}
