...
\subsection{Hiệu quả tham số (Parameter Efficiency)}
Chúng tôi đánh giá tính kinh tế của phương pháp thông qua tỷ lệ tham số học được ($P_{learnable}$) so với tổng số tham số của backbone ($P_{total}$):
\begin{equation}
    \text{Efficiency Ratio} = \frac{M \times D}{P_{total}}
\end{equation}
Trong đó $M$ là độ dài ngữ cảnh và $D$ là số chiều embedding. Với APT, tỷ lệ này thường cực kỳ nhỏ (vài nghìn tham số so với hàng trăm triệu tham số của CLIP), thể hiện ưu điểm vượt trội về mặt tài nguyên so với các phương pháp Adversarial Fine-tuning truyền thống.
