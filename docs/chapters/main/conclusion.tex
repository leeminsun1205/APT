...
Trong đồ án này, chúng tôi đã nghiên cứu về tính bền vững (robustness) của các mô hình Đa phương thức quy mô lớn (Large-scale Vision-Language Models) như CLIP trước các tấn công đối kháng. Thay vì sử dụng các phương pháp Adversarial Training truyền thống - vốn yêu cầu tài nguyên tính toán lớn để cập nhật toàn bộ trọng số mô hình - chúng tôi đã triển khai phương pháp \textbf{Adversarial Prompt Tuning (APT)}.

Các kết quả nghiên cứu chính bao gồm:
\begin{enumerate}
    \item \textbf{Hiệu quả thực nghiệm:} APT đã chứng minh khả năng cải thiện cả độ chính xác trên dữ liệu sạch và độ bền vững trước tấn công PGD. Đặc biệt, trên bộ dữ liệu CIFAR-100, APT đạt mức tăng trưởng Robust Accuracy đáng chú ý (+26.4\%) so với baseline, đồng thời nâng cao Clean Accuracy thêm +6.9\%.
...
\begin{enumerate}
    ...
    \item \textbf{Tấn công Black-box:} Đánh giá khả năng phòng thủ của APT trước các loại tấn công Black-box (khi kẻ tấn công không truy cập được kiến trúc mô hình), nhằm phản ánh chính xác hơn các kịch bản thực tiễn.
\end{enumerate}