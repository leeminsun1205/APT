...
\begin{table}[H]
  ...
  \begin{tabular}{|l|cc|cc|}
    ...
    \textit{Improvement}             & \textit{+5.0\%}                        & \textit{+21.8\%}                        & \textit{+6.9\%} & \textit{+26.4\%} \\ \hline
  \end{tabular}
\end{table}

\textbf{Nhận xét:}
\begin{itemize}
  \item \textbf{Robustness:} APT đạt được mức tăng trưởng vượt trội về độ bền vững. Trên CIFAR-100, độ chính xác đối kháng (Robust Accuracy) tăng hơn gấp 3 lần so với Zero-shot (từ 12.1\% lên 38.5\%). Điều này chứng tỏ prompt học được đã giúp mô hình tập trung vào các đặc trưng ngữ nghĩa cốt lõi, ít bị ảnh hưởng bởi nhiễu nền.
  \item \textbf{Trade-off:} Khác với Adversarial Training truyền thống thường làm giảm độ chính xác trên dữ liệu sạch (Clean Accuracy), APT lại cải thiện đồng thời cả Clean Acc (+5.0\% trên CIFAR-10). Điều này cho thấy tính ưu việt của việc tối ưu hóa trong không gian prompt thay vì không gian trọng số.
  \item \textbf{So với Linear Probe:} Linear Probe tuy đạt Clean Acc cao nhưng hoàn toàn "vỡ trận" trước tấn công đối kháng (Robust Acc gần như bằng 0). APT khắc phục triệt để điểm yếu này.
\end{itemize}
...
Các thực nghiệm trên diện rộng đã khẳng định tính hiệu quả, bền vững và tiết kiệm chi phí của phương pháp APT. Nó không chỉ vượt qua các baseline về độ chính xác mà còn giải quyết bài toán robustness một cách thanh lịch mà không cần can thiệp sâu vào cấu trúc mô hình.
