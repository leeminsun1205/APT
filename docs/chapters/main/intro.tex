...
Xuất phát từ những thách thức thực tế đó, đề tài này tập trung nghiên cứu một hướng tiếp cận mới, nhẹ nhàng hơn nhưng vẫn đảm bảo hiệu quả: \textbf{Prompt Tuning}. Thay vì thay đổi trọng số của mô hình (Model Weights), chúng ta sẽ giữ nguyên chúng và chỉ tối ưu hóa đầu vào văn bản (Text Prompt). Cụ thể, đề tài đề xuất phương pháp \textbf{Adversarial Prompt Tuning (APT)} - học một prompt tối ưu giúp mô hình CLIP chống lại các tấn công đối kháng một cách hiệu quả.
...
\begin{enumerate}
    ...
    \item Đề xuất và triển khai thuật toán APT để tìm kiếm các vector ngữ cảnh (context vectors) tối ưu.
...
\begin{enumerate}
    ...
    \item \textbf{Đề xuất phương pháp APT:} Chúng tôi giới thiệu một quy trình huấn luyện đơn giản gọi là Adversarial Prompt Tuning. Phương pháp này chỉ cần học một số lượng tham số rất nhỏ (vài nghìn tham số) so với việc fine-tune toàn bộ mô hình (hàng trăm triệu tham số), nhưng lại mang lại hiệu quả vượt trội.
