...
\section*{Đặt vấn đề}
Trong những năm gần đây, cộng đồng trí tuệ nhân tạo đã chứng kiến sự phát triển nhanh chóng của các Mô hình Đa phương thức Ngôn ngữ-Thị giác (Vision-Language Models - VLMs). Điển hình là CLIP (Contrastive Language-Image Pre-training) \cite{radford2021learning}, một mô hình được huấn luyện trên 400 triệu cặp ảnh-văn bản thu thập từ internet. CLIP và các biến thể của nó đã chứng minh khả năng tổng quát hóa (generalization) vượt trội, cho phép thực hiện các tác vụ nhận dạng ảnh zero-shot (không cần huấn luyện lại) trên nhiều miền dữ liệu khác nhau với độ chính xác cao.

Tuy nhiên, cùng với những tiến bộ đó là những lo ngại về tính an toàn và bảo mật của các mô hình này. Các nghiên cứu gần đây đã chỉ ra rằng, mặc dù VLMs rất mạnh mẽ trên dữ liệu sạch, chúng lại cực kỳ nhạy cảm với các \textit{tấn công đối kháng} (adversarial attacks). Các hình ảnh đối kháng - được tạo ra bằng cách thêm các nhiễu loạn nhỏ vô hình với mắt người vào ảnh gốc - có khả năng gây ra sai lệch trong kết quả phân loại của mô hình với độ tin cậy rất cao. Sự thiếu vững chắc (robustness) này đặt ra thách thức đáng kể cho việc triển khai VLMs trong các ứng dụng thực tế đòi hỏi độ tin cậy cao, như hệ thống nhận diện khuôn mặt, xe tự lái, hay kiểm duyệt nội dung y tế.
...
Xuất phát từ những thách thức thực tế đó, đề tài này tập trung nghiên cứu chi tiết một hướng tiếp cận hiệu quả đã được giới thiệu gần đây: \textbf{Prompt Tuning}. Thay vì thay đổi trọng số của mô hình (Model Weights), chúng ta sẽ giữ nguyên chúng và chỉ tối ưu hóa đầu vào văn bản (Text Prompt). Cụ thể, đề tài nghiên cứu việc tái cài đặt và đánh giá phương pháp \textbf{Adversarial Prompt Tuning (APT)} - kỹ thuật học một prompt tối ưu giúp mô hình CLIP chống lại các tấn công đối kháng.
...
\begin{enumerate}
    ...
    \item \textbf{Kết quả thực nghiệm khả quan:} Trên các thử nghiệm rộng rãi với 15 bộ dữ liệu, APT cho thấy khả năng cải thiện trung bình $+13\%$ độ chính xác trên ảnh sạch và $+8.5\%$ độ chính xác trên ảnh đối kháng so với baseline sử dụng các prompt thủ công (hand-crafted prompts). Kết quả này khẳng định triển vọng cho việc triển khai các mô hình AI an toàn với chi phí phù hợp.
\end{enumerate}
