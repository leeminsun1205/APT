\begin{figure*}
    \centering
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/legend.pdf}
    \end{subfigure}
    
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/oxford_flowers.pdf}
    % \end{subfigure}
    % % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/eurosat.pdf}
    % \end{subfigure}
    % % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/imagenet.pdf}
    % \end{subfigure}
    % % \hfill


    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/oxford_pets.pdf}
    % \end{subfigure}
    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/ucf101.pdf}
    % \end{subfigure}
    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/caltech101.pdf}
    % \end{subfigure}
    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/dtd.pdf}
    % \end{subfigure}
    % \hfill

    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/stanford_cars.pdf}
    % \end{subfigure}
    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/sun397.pdf}
    % \end{subfigure}
    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/food101.pdf}
    % \end{subfigure}
    % \hfill
    % \begin{subfigure}{.24\linewidth}
    %     \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/fgvc_aircraft.pdf}
    % \end{subfigure}
    % \hfill
    
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/Average.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/oxford_flowers.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/imagenet.pdf}
    \end{subfigure}
    \hfill

    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/caltech101.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/oxford_pets.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/stanford_cars.pdf}
    \end{subfigure}
    \hfill

    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/food101.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/fgvc_aircraft.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/sun397.pdf}
    \end{subfigure}
    \hfill

    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/dtd.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/eurosat.pdf}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.27\linewidth}
        \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps1/ucf101.pdf}
    \end{subfigure}
    \hfill

    \caption{The in-distribution performance on 11 datasets and the averaged performance under different shots. $\epsilon=1/255$ and M = 16.}
    \label{fig: shot results eps 1}
    
\end{figure*}

% \begin{figure*}
%     \centering
%     \begin{subfigure}{\linewidth}
%         \centering
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/legend.pdf}
%     \end{subfigure}
    
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/Average.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/oxford_flowers.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/imagenet.pdf}
%     \end{subfigure}
%     \hfill

%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/caltech101.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/oxford_pets.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/stanford_cars.pdf}
%     \end{subfigure}
%     \hfill

%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/food101.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/fgvc_aircraft.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/sun397.pdf}
%     \end{subfigure}
%     \hfill

%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/dtd.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/eurosat.pdf}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}{.27\linewidth}
%         \includegraphics[width=\linewidth]{imgs/shotresult/vitb32-eps4/ucf101.pdf}
%     \end{subfigure}
%     \hfill

%     \caption{The performance of our methods on 11 datasets when trained with different shots under $\epsilon=4/255$.}
%     \label{fig: shot result all datasets eps 4}
    
% \end{figure*}


